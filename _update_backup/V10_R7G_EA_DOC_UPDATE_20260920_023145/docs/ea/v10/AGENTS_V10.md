...
<!-- V10_R7_SIZING_SHADOW_OVERRIDE_20260917_START -->
## 0F. Current R7 sizing-shadow continuation — 2026-09-17

After the R3/R4/R5 causal-ML material, also read:
...
```text
preceding HA / MTF flow is useful context
!= reliable direct next-HA authority across Gold regimes

R4 remains base participation research.
R7G only asks whether an existing R4 1-unit Child may be upgraded to 3 units.
```

Frozen R7G shadow semantics:

```text
R4 weight == 1
AND R5 EV > 0
AND recent already-exited eligible Child mean R
    over the last 180 completed H4 bars > 0
-> shadow 1 -> 3 unit upgrade

otherwise preserve R4 action
```

This is consumed-data shadow research only.
Do not promote `180 H4`, R7G, or the post-hoc `no-k3p-SHORT` diagnostic to strategy authority.
Do not reopen next-HA / model-family / threshold scans on the same consumed 2024-2026 evidence.
