...
setlocal
cd /d "%~dp0.."
for /f %%i in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddTHHmmss"') do set RUN_ID=gemini_v4_aug18_22_%%i
echo [V4] Gemini Aug 18-22. Run Sol_Replay_Week.cmd first.
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\run_python_sandbox.ps1 scripts\mentor_ai_replay_v4.py run --run-id "%RUN_ID%" --decision-provider gemini --benchmark-truth "output\mentor_aug18_22_truth_v1\funnel_truth.json" --start 2025-08-18T00:00:00Z --end 2025-08-23T00:00:00Z || goto :failed
...
exit /b 0
:failed
echo [FAILED] V4 weekly replay stopped without a reproducibility claim.
pause
exit /b 1
