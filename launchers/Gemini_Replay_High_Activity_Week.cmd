...
setlocal
cd /d "%~dp0.."
for /f %%i in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddTHHmmss"') do set RUN_ID=gemini_v4_oct28_31_%%i
echo [V4] Gemini corrected protocol period: Oct 28-31. Run Sol_Replay_High_Activity_Week.cmd first.
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\run_python_sandbox.ps1 scripts\mentor_ai_replay_v4.py run --run-id "%RUN_ID%" --decision-provider gemini --benchmark-truth "output\mentor_oct28_31_protocol_truth_v2\funnel_truth.json" --start 2025-10-28T00:00:00Z --end 2025-11-01T00:00:00Z || goto :failed
...
exit /b 0
:failed
echo [FAILED] The current high-activity benchmark conflicts with the V4 H1 PLAN/trigger contract. No reproducibility claim was made.
pause
exit /b 1
