...
def canonical(rows):
    out=[]
    for r in rows:
        if r.get("event","").startswith("D154G_"): continue
        x=dict(r)
        d=x.get("detail","")
...
def main()->int:
    ...
    d154g_on=sum(1 for r in b if r.get("event","").startswith("D154G_"))
    d154g_off=sum(1 for r in a if r.get("event","").startswith("D154G_"))
    if d154g_off!=0:
        print(f"D154G NON-INTERFERENCE PARITY: FAIL | OFF contains D154G rows={d154g_off}"); return 2
    if ca!=cb:
        print(f"D154G NON-INTERFERENCE PARITY: FAIL | canonical OFF={len(ca)} ON={len(cb)}")
        for i,(x,y) in enumerate(zip(ca,cb)):
            if x!=y:
                print("first_diff_index=",i); print("OFF",x); print("ON ",y); break
        if len(ca)!=len(cb): print("canonical length differs")
        return 2
    print(f"D154G NON-INTERFERENCE PARITY: PASS | canonical_rows={len(ca)} | d154g_on_rows={d154g_on}")
    return 0
