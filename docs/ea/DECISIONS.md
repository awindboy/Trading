...
## D-156 ??Open V3 raw-market-data research line

Status: `ACTIVE / 2026-08-24`

Decision:
- Create V3 as a separate active research line rather than continuing to mine
  filters inside the sparse V2 fill population.
- V1 remains frozen historical deterministic control.
- V2 is paused and preserved as the continuation/SP/Entry/execution research control.
- V3 begins from raw broker market data and may redesign the strategy layer while
  preserving causality, no-look-ahead, execution realism, discovery/validation
  separation and final MT5 confirmation.
- V3 does not require literal replication of every discretionary mentor rule.
  Mentor concepts become causal priors/hypotheses rather than immutable implementation
  constraints.
- V3 discovery population is the broad outcome-blind market event/opportunity universe,
  not only current EA fills.
- 2023-2025 are allocated to the initial V3 discovery/development laboratory.
- 2022 is reserved as the V3 validation vault unless later proven contaminated for the
  exact V3 hypothesis.
- 2021 remains untouched.
- Initial input is GOLD# M1 broker-server history; higher timeframes are rebuilt
  causally offline. Tick data is requested only after candidate strategies survive
  fast M1 replay.
- ML/statistical models may be used as hypothesis-discovery tools. The default final
  EA target remains interpretable/deterministic unless a future decision explicitly
...
## D-172 — Close V5-002/V5-003 generic breakout-direction route

Status: `FROZEN NEGATIVE`
Date: `2026-08-27`

Balance/contraction descriptors did not stably predict breakout direction. Cross-scale trendability did not rescue
remaining continuation. Pre-event expansion/trendiness described future movement intensity more reliably than direction.

Do not reopen these results through a preferred range length, contraction threshold or confirmation window.

## D-173 — Success-first setup cycle does not inherit trader reputation as strategy authority

Status: `ACTIVE / FROZEN LESSON`
Date: `2026-08-27`

Holy Grail, Turtle Soup, Anti, Momentum Pinball, 80-20 and related source-grounded tests were treated as hypotheses,
not as proof.

Most mechanical reproductions failed the project's joint WR / payoff / expectancy target.

This supports the V5 rule:

```text
successful trader source
...
## D-174 — First Cross 240m becomes the discovery-selected scale before exact replay

Status: `FROZEN DISCOVERY HISTORY`
Date: `2026-08-27`

V5-027B pre-registered 240m and 480m as the two higher intraday bridge scales.

Structural-retest discovery:

```text
240m  N342 / WR53.22% / avg positive1.296R / EV+0.203R
480m  N197 / WR49.24% / avg positive1.297R / EV+0.116R
```

240m was selected once and consumed as discovery.

No later timeframe search may treat 240m as pristine.

## D-175 — Freeze V5-030A as the first V5 development PASS

Status: `ACTIVE FROZEN VALIDATION CANDIDATE`
Date: `2026-08-27`
...
```text
240m First Cross
+ first causal 3-bar price pivot
+ stop-entry beyond completed confirmation bar
+ structural pivot stop
+ 50% at +1R
+ runner stop to BE
+ runner exit at adverse completed 240m EMA20 close or slow-line zero reversal
```
...
spread-adjusted EV +0.148R
```

All three pooled years are positive. Three of four development markets are positive. Every leave-one-market/year pooled
EV remains positive.

This is `DEVELOPMENT PASS`, not production authority.

## D-176 — Reject V5-031/V5-032 rescue filters; stop development mining

Status: `ACTIVE / FROZEN`
Date: `2026-08-27`

Daily 3/10 context failed temporal stability.

The pre-registered ATR adequacy subset had:

```text
N155 / WR61.29% / avg positive0.993R / EV+0.212R
```

and failed its own sample/payoff/consumed-diagnostic gate.

Do not add either filter to V5-030A. Do not create an USDJPY veto.

## D-177 — Open V5-034A external validation and keep GOLD# 2021 closed

Status: `PRE-REGISTERED VALIDATION`
Date: `2026-08-27`
...
The candidate must run unchanged on all four.

GOLD# 2021 remains untouched until V5-034A receives an explicit external-validation PASS.

External validation failure closes V5-034A without filter rescue.



## D073-D080 — V6 close / V7 phase transition (2026-08-30)

### D073 — V6 combined architecture fails final external/temporal validation
The frozen H+L1+L2 architecture is not promoted.
V6 becomes a historical research control.

### D074 — Close previous mentor-style strategy development
No further H/L1/L2 strategy-semantic development by default.
Only reproducibility/archival corrections remain allowed.

### D075 — V7 active generation
V7 is named and activated as the Double-B / 더캔이지추격깨 / KTR research generation.

### D076 — Double-B is event detection, not direction
Upper/lower Double-B side and outside-band close do not independently authorize LONG/SHORT.

### D077 — Context may remain discretionary
Do not force unfaithful formulas for support/resistance, trendline, candle meaning, or other
context merely to produce a numerical score.

### D078 — KTR is a distance/force coordinate system
SL/TP multipliers remain event/context dependent.
No universal KTR multiplier table is current authority.

### D079 — 24 reverse-engineered events are consumed
The blind pilot and outcome-informed reverse engineering set cannot be reused as validation.
The +89.03R hindsight result has zero claim-grade performance authority.
