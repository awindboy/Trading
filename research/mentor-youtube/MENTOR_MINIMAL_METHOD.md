...
| optional child audit 프레임 | HTF OB 접촉 이후 반응에서 새 causal child OB가 보이는가? | M30, M15, M5 |
| trigger 프레임 | Root 반응의 sweep과 추세 전환이 어디서 확인되는가? | M5, M1 |
...
Post-contact child가 parent OB 안에 있거나 일부 경계가 벗어나더라도, **동일한 post-contact reaction을 설명하는 causal relation**이 먼저 성립해야 audit observation으로 기록할 수 있다.
Child는 선택적 context/audit 정보다. 확인되지 않아도 Root setup은 계속되며, 여러 child가 보여도 하나를 강제로 고르지 않는다.
실제 entry/SL geometry는 child와 무관하게 CHoCH displacement M1 FVG 규칙이 담당한다.
...
HTF Root 접촉 이후 Root 반응 맥락에서 유효한 sweep을 확인한 뒤, M5/M1에서 기존 추세의 live swing을 반대 방향 몸통 종가로 깨고 후속 가격이 그 방향으로 전달되는지 확인한다. 이 변화가 CHoCH다. Optional child 관찰 여부는 이 trigger의 선행조건이 아니다.

현재 deterministic V1의 **운영 계약(D-127)** 은 detector와 scenario sequence를 분리한다.

- M1 sweep detector는 M1 bar open에 이미 causally known인 active liquidity에 기존 penetration + same-bar recovery geometry만 적용한다.
- 이 detector에는 Root 재교차, scenario, child, ATR/point/N-bar/score 같은 strategy filter를 넣지 않는다.
- LONG/SHORT 방향 호환성은 sweep 구조 자체의 정의가 아니라 scenario sequence에서 적용한다.
- Root contact 이후의 첫 direction-compatible detected sweep이 `Sweep` 단계를 만족한다. Root-contact bar 자체는 closed OHLC로 `contact → sweep` 순서를 증명할 수 없으므로 scenario 단계에는 사용하지 않는다.
- M1 CHoCH는 기존 structure detector가 독립적으로 판정하며, scenario는 sweep 시점의 M1 trend/protected swing을 다시 freeze하거나 재심사하지 않는다.
- Scenario는 단순히 `Root contact → Sweep → later same-direction CHoCH` 순서만 확인한다.

이 세부 사항은 스승님의 직접 문구가 아니라 causal replay를 위한 deterministic architecture decision이다. 구조 하나마다 또 다른 다중 strategy gate를 중첩하지 않는다.

별도 BOS는 전환 신뢰도를 높이는 확인이지만 영상 전체의 필수 조건은 아니다.
기본 실행 구역은 CHoCH displacement가 새로 만든 M1 FVG다. HTF Root가 가격이 반응할 원인 위치를 제공하며, optional post-contact child는 그 반응을 추가 설명하는 audit 정보일 뿐이다.
의미 있는 CHoCH가 있어도 같은 sweep-to-CHoCH leg에 fresh FVG가 없으면 구조 전환만 기록하고 최초 포지션은 진입하지 않는다.

- 기본형: pre-existing HTF OB -> HTF OB contact -> sweep -> M1 CHoCH
  -> CHoCH displacement FVG -> FVG retest
- 확인형: pre-existing HTF OB -> HTF OB contact -> sweep -> M1 CHoCH
  -> 별도 BOS -> BOS displacement FVG -> FVG retest
- OB 정밀형: 전환 FVG 없이 OB만 재접촉하는 사례는 기본형과 섞지 않고 별도 연구 원장으로만 남긴다.

Post-contact child는 execution authority가 아니므로 별도 child retest를 current baseline의 필수 단계로 요구하지 않는다.
...
4. 사전에 정한 HTF swing OB는 무엇이고 가격은 그 Root에 실제로 도달했는가? Optional child가 관찰됐다면 무엇인가?
5. 유동성 sweep과 작은 시간봉 CHoCH가 실제로 발생했는가?
6. entry FVG는 Root 반응의 valid sweep 이후 CHoCH를 만든 M1 displacement에 속하며, valid FVG 중 가장 넓고 meaningful CHoCH 확정 이후의 first retest를 사용하고 있는가?
...
- HTF Root actual contact
- optional post-contact causal LTF child audit/context
- adaptive MTF 역할 분담
- sweep 뒤 LTF 추세 전환
...
5. `STRUCTURAL_REACTION` liquidity를 Root reaction에서 언제 새로 생성·귀속할지

일반 `EXTERNAL_SWING / DEFENDED_RANGE_EDGE`의 strategic sweep ownership은
D-126 operational contract로 고정되었다. 남은 항목들은 필터 튜닝 문제가 아니라
매매 모델의 서로 다른 변형 또는 아직 미확정된 세부 계약이다.
