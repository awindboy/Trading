#!/usr/bin/env python3
"""Apply/recover D-146 continuation-state shadow audit on exact GitHub HEAD f0a9be86.

This v2 installer is fail-closed and idempotent across exact D-146 partial states:
it accepts each target only when it equals either committed HEAD or the exact
D-146 generated output. Unknown local edits are still rejected.
"""
...
def head_text(repo: Path, rel: str) -> str:
    proc = subprocess.run([GIT, "show", f"HEAD:{rel}"], cwd=str(repo), stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
    if proc.returncode != 0:
        raise RuntimeError(f"Unable to read HEAD:{rel}: {decode_process_output(proc.stderr or proc.stdout)}")
    try:
        return normalize_text(proc.stdout.decode("utf-8-sig"))
    except UnicodeDecodeError as exc:
        raise RuntimeError(f"HEAD:{rel} is not UTF-8 text: {exc}")


def classify_working_file(repo: Path, rel: str, baseline: str, expected: str) -> str:
    current = normalize_text(read(repo / rel))
    if current == normalize_text(baseline):
        return "BASELINE"
    if current == normalize_text(expected):
        return "D146_ALREADY_APPLIED"
    diff = subprocess.run([GIT, "diff", "--", rel], cwd=str(repo), stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
    detail = decode_process_output(diff.stdout)
    if len(detail) > 1800:
        detail = detail[:1800] + " ..."
    raise RuntimeError(
        f"Unexpected local edits in {rel}. The recovery installer accepts only exact GitHub HEAD or exact D-146 output.\n{detail}"
    )
...
def build_expected_outputs(repo: Path) -> tuple[dict[str, str], dict[str, str]]:
    baselines = {rel: head_text(repo, rel) for rel in EXPECTED_BLOBS}
    expected = dict(baselines)
    expected["mt5/experts/MentorDeterministicV1EA.mq5"] = normalize_text(
        patch_ea_identity(baselines["mt5/experts/MentorDeterministicV1EA.mq5"])
    )
    expected["mt5/experts/EdgeAuditV1.mqh"] = normalize_text(
        patch_edge(baselines["mt5/experts/EdgeAuditV1.mqh"])
    )

    # Reuse the exact document patch logic against a temporary copy of HEAD.
    # This makes the installer idempotent without weakening local-state checks.
    with tempfile.TemporaryDirectory(prefix="d146_expected_") as td:
        temp = Path(td)
        for rel in EXPECTED_BLOBS:
            if rel.startswith("docs/ea/"):
                write(temp / rel, baselines[rel])
        patch_docs(temp)
        for rel in EXPECTED_BLOBS:
            if rel.startswith("docs/ea/"):
                expected[rel] = normalize_text(read(temp / rel))

    return baselines, expected
...
def main() -> int:
    try:
        ...
        # Generate the exact final D-146 files from the committed GitHub HEAD,
        # then accept either pristine HEAD or an exact partial D-146 file.
        # Any third state still fails closed. This recovers safely from a prior
        # interrupted/partial D-146 application.
        baselines, expected = build_expected_outputs(repo)
        states = {}
        for rel in EXPECTED_BLOBS:
            states[rel] = classify_working_file(repo, rel, baselines[rel], expected[rel])

        new_tool_path = repo / NEW_TOOL
        tool_expected = normalize_text(read(PAYLOAD_TOOL))
        if new_tool_path.exists():
            if normalize_text(read(new_tool_path)) != tool_expected:
                raise RuntimeError(
                    f"Unexpected existing file: {NEW_TOOL}. It is not the exact D-146 analyzer payload."
                )
            tool_state = "D146_ALREADY_APPLIED"
        else:
            tool_state = "MISSING"

        print("D-146 recovery preflight accepted local state:")
        for rel in EXPECTED_BLOBS:
            print(f"  {states[rel]:20s} {rel}")
        print(f"  {tool_state:20s} {NEW_TOOL}")

        # All state validation is complete before the first write.
        for rel, final_text in expected.items():
            write(repo / rel, final_text)
        write(new_tool_path, tool_expected)
