...
PROBE_VERSION = "2026-09-01-v4-utc-wallclock-alignment-audit"

def parse_args():
    ap=argparse.ArgumentParser()
    ap.add_argument("--resolved-ledger", required=True)
    ap.add_argument("--symbol", default="GOLD#")
    ap.add_argument("--m1-source", required=True,
                    help="Unified GOLD M1 CSV used to build the N1 ledger; used only for outcome-blind time-alignment audit")
    ap.add_argument("--server-tz", default=None,
                    help="DEPRECATED in V4 and ignored. Ledger/M1 wall-clock is already aligned to MT5 UTC timestamps.")
    ap.add_argument("--out", default="V8_N2_NORMALIZED_TICK_VOTER_RESULTS")
    ap.add_argument("--terminal-path", default=None)
    return ap.parse_args()

def ledger_wallclock_to_utc(ts: pd.Timestamp):
    """Treat ledger/M1 wall-clock value as the MT5 UTC clock value.

    Empirical alignment audit on the unified M1 source showed that applying
    Europe/Helsinki (+2/+3) localization shifted tick activity by exactly
    -2h in winter and -3h in summer. Therefore V4 does not localize the
    ledger timestamp before MT5 lookup.
    """
    if ts.tzinfo is not None:
        return ts.tz_convert("UTC").to_pydatetime()
    return ts.to_pydatetime().replace(tzinfo=timezone.utc)

def load_m1_tickvol(path):
    m=pd.read_csv(path, sep="\t", usecols=["<DATE>","<TIME>","<TICKVOL>"])
    m["dt"]=pd.to_datetime(m["<DATE>"]+" "+m["<TIME>"], format="%Y.%m.%d %H:%M:%S")
    m["tickvol"]=pd.to_numeric(m["<TICKVOL>"], errors="coerce").fillna(0.0)
    return dict(zip(m["dt"],m["tickvol"]))

def source_prev5_tickvol(decisions, tickvol_map, placebo_minutes=0):
    vals=[]
    for d in decisions:
        end=pd.Timestamp(d)-pd.Timedelta(minutes=placebo_minutes)
        s=0.0
        for k in range(1,6):
            s += float(tickvol_map.get(end-pd.Timedelta(minutes=k),0.0))
        vals.append(s)
    return np.asarray(vals,dtype=float)

def alignment_audit(df, m1_tickvol_map, out):
    """Outcome-blind timing audit using raw-tick counts vs source M1 tick volume."""
    src_a=source_prev5_tickvol(df["decision"],m1_tickvol_map,0)
    src_p=source_prev5_tickvol(df["decision"],m1_tickvol_map,10)
    rows=[]
    for name,tick_col,src in [
        ("aligned","a300_n",src_a),
        ("placebo","p300_n",src_p),
    ]:
        tick=pd.to_numeric(df[tick_col],errors="coerce").to_numpy(float)
        ok=np.isfinite(tick) & (src>0)
        corr=float(np.corrcoef(tick[ok],src[ok])[0,1]) if ok.sum()>10 else np.nan
        tick_missing=~np.isfinite(tick)
        src_zero=src<=0
        rows.append({
            "series":name,
            "N":len(df),
            "N_corr":int(ok.sum()),
            "corr_tickcount_vs_m1_tickvol":corr,
            "source_zero_rate":float(src_zero.mean()),
            "tick_missing_rate":float(tick_missing.mean()),
            "zero_missing_match_rate":float((src_zero==tick_missing).mean()),
        })
    aud=pd.DataFrame(rows)
    aud.to_csv(out/"time_alignment_audit.csv",index=False)
    # Strong, outcome-blind gate. The uploaded V3 misaligned run was ~0.54;
    # correct winter/summer offsets separately were ~0.99.
    aligned_corr=float(aud.loc[aud.series=="aligned","corr_tickcount_vs_m1_tickvol"].iloc[0])
    if not np.isfinite(aligned_corr) or aligned_corr < 0.90:
        (out/"STATUS.txt").write_text(
            f"FAIL_CLOSED_TIME_ALIGNMENT\naligned_corr={aligned_corr}\n",
            encoding="utf-8")
        raise SystemExit(
            f"FAIL-CLOSED time alignment: aligned 5m tick-count vs M1 tick-volume correlation "
            f"{aligned_corr:.3f} < 0.90. See time_alignment_audit.csv")
    return aud
...
def main():
    ...
    if args.server_tz:
        print(f"NOTE: --server-tz={args.server_tz!r} is ignored in V4; no timezone shift is applied.")
    m1_tickvol_map=load_m1_tickvol(args.m1_source)
    ...
    try:
        for i,r in enumerate(led.itertuples(index=False),1):
            dec_utc=ledger_wallclock_to_utc(pd.Timestamp(r.decision))
    ...
    # Time alignment MUST pass before any direction performance is considered.
    alignment=alignment_audit(df,m1_tickvol_map,out)
    print("Time alignment audit:")
    ...
    summary={"population_counts":counts,
             "time_alignment_audit":alignment.to_dict(orient="records"),
